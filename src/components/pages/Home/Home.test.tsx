...
describe("HomePage", () => {
  ...
  test("renders stable labelled outputs for the current scaffold", () => {
    ...
    expect(html).toContain('<span class="stat-label">Armour class</span>');
    expect(html).toContain('<strong class="stat-value">18</strong>');
    expect(html).toContain('<span class="stat-label">Hit points</span>');
    expect(html).toContain('<strong class="stat-value">35</strong>');
    expect(html).toContain('<span class="stat-label">Initiative</span>');
    expect(html).toContain('<strong class="stat-value">+2</strong>');
    expect(html).toContain('<span class="stat-label">Speed</span>');
    expect(html).toContain('<strong class="stat-value">30 ft</strong>');
  });
