...
5. Replace rehearsal player records and load Friday character shells as described in
   [Hosted Friday Player Runbook](./hosted-friday-player-runbook.md):

```bash
HOSTED_PLAYERS_BACKUP_CONFIRMED=1 bun run hosted:players
```

6. If the operator source is OCR Markdown, convert it to private-rules YAML:
...
7. Import the private rules with the backup confirmation:

```bash
PRIVATE_RULES_BACKUP_CONFIRMED=1 bun run import:rules:private -- /data/private-rules
```

8. Run the coverage report after import:

```bash
bun run rules:coverage:rovnost -- /data/private-rules
```

9. Check the hosted readiness endpoint:

```bash
bun run hosted:check -- <hosted-url>
```

10. Sign in as the Game Master and Lynott player, open `/sheet/lynott`, and spot-check private rule
