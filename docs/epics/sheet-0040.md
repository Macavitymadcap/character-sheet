# Epic sheet-0040: Hyper-Dank Platform Adoption
...
This epic follows the full SRD 5.1 rules/functionality epic (`sheet-0020`) and Railway deployment
epic (`sheet-0030`). It is a platform-alignment epic, not a new gameplay, campaign-content, or
deployment epic. Character Sheet should adopt only the parts of Hyper-Dank that are already
app-agnostic and documented: shared components, shared HTTP helpers, and database migration
primitives where they fit the deployed app boundary. Character Sheet keeps ownership of D&D domain
language, SRD import, campaign permissions, route design, schema shape, seed data, and gameplay
workflow decisions.
...
- No new player, Game Master, admin, rules, SRD, or campaign features.
- No Railway deployment, Postgres migration, hosted asset storage, production secret, or seeded
  account work in this epic.
...
- D&D-specific table definitions, seeds, and repositories
...
| Ticket | Purpose |
| --- | --- |
| `sheet-0041` | Add Hyper-Dank package installation, shared stylesheet wiring, and Character Sheet compatibility test scaffolding. |
| `sheet-0042` | Migrate existing generic atoms and molecules to `@macavitymadcap/hyper-dank-components`. |
| `sheet-0043` | Adopt `@macavitymadcap/hyper-dank-http` for generic form parsing, validation helpers, and response helpers. |
| `sheet-0044` | Evaluate and adopt `@macavitymadcap/hyper-dank-database` only where it improves deployed database and migration boundaries. |
| `sheet-0045` | Expand compatibility, accessibility, and screenshot coverage around the shared-component migration. |
| `sheet-0046` | Update docs, verification flow, and final acceptance notes for the Hyper-Dank consumer workflow. |

## Branch Strategy

Create `sheet-0040` from the latest `main` after the `sheet-0020` and `sheet-0030` epics land.
Open the planning pull request into `main`. Once accepted, keep or recreate `sheet-0040` as the
epic integration branch. Tickets `sheet-0041` through `sheet-0046` should branch from
`sheet-0040`, open pull requests back into `sheet-0040`, and be squash-merged there before the
epic lands on `main`.
...
- Character Sheet may need a repeatable local package-install strategy such as tarball packs or
  workspace linking; `sheet-0041` should pick one documented path and keep it easy to rerun.
...
- The database package may remain a partial fit even after deployment. If so, document the choice
  and keep Character Sheet-owned database code where the abstraction would obscure SRD or campaign
  behaviour.
